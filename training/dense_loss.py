...
class DenseDetectionLoss(nn.Module):
    ...
    def forward(
        ...
        quality_levels: list[torch.Tensor] | None = outputs.get("quality")  # type: ignore[assignment]
        ...
        for cls_map, box_map, stride, size_range, qual_map in level_iter:
            ...
            if qual_map is not None:
                flat_qual.append(qual_map.float().squeeze(1).reshape(batch_size, -1))
        ...
        for batch_index, target in enumerate(targets):
            ...
            if pos_mask.any():
                ...
                if pred_qual is not None:
                    q_logits_pos = pred_qual[batch_index][pos_mask]
